...
var statistic = {
	...
	numer_of_republicans: 0,
	number_of_independents: 0
};

function getHeaderHtml() {
	return document.getElementById("header-glance").innerHTML = "<tr><th>Party</th><th>No. of Resp</th><th>%voted w/party</th></tr>";
};
getHeaderHtml();

function getHeaderHtml1() {
	return document.getElementById("header-engaged").innerHTML = "<tr><th>leandro</th><th>No. of Resp</th><th>%voted w/party</th></tr>";
};

getHeaderHtml1();

var members1 = dataSenate.members;

statistic.number_of_independents = members1.filter(miembro => miembro.party == 'I').length;
statistic.number_of_democrats = members1.filter(miembro => miembro.party == 'D').length;
statistic.numer_of_republicans = members1.filter(miembro => miembro.party == 'R').length;
//console.log(statistic);


var sumatoriaDeVotesWithPartyI = (members1.filter(miembro => miembro.party == 'I').map(miembro => miembro.votes_with_party_pct).reduce((primerMiembro, segundoMiembro) => primerMiembro + segundoMiembro, 0) / members1.length).toFixed(2);

var sumatoriaDeVotesWithPartyD = (members1.filter(miembro => miembro.party == 'D').map(miembro => miembro.votes_with_party_pct).reduce((primerMiembro, segundoMiembro) => primerMiembro + segundoMiembro, 0) / members1.length).toFixed(2);

var sumatoriaDeVotesWithPartyR = (members1.filter(miembro => miembro.party == 'R').map(miembro => miembro.votes_with_party_pct).reduce((primerMiembro, segundoMiembro) => primerMiembro + segundoMiembro, 0) / members1.length).toFixed(2);

var sumaPorcentaje = parseFloat(sumatoriaDeVotesWithPartyD) + parseFloat(sumatoriaDeVotesWithPartyI) + parseFloat(sumatoriaDeVotesWithPartyR);
console.log(sumaPorcentaje);

function getColumnsHtml() {

	var tabla = '<tr><td>Democrats</td><td>' + statistic.number_of_democrats + '</td><td>' + sumatoriaDeVotesWithPartyD + '%</td></tr>' +
		'<tr><td>Republicans</td><td>' + statistic.numer_of_republicans + '</td><td>' + sumatoriaDeVotesWithPartyR + '%</td></tr>' +
		'<tr><td>Indepents</td><td>' + statistic.number_of_independents + '</td><td>' + sumatoriaDeVotesWithPartyI + '%</td></tr>' +
		'<tr><td>Total</td><td>' + members1.length + '</td><td>' + sumaPorcentaje + '%</td></tr>';
...
function renderRows() {
	var html = getColumnsHtml();
	document.getElementById("glance").innerHTML = html;  
};
...
function bottomDiez(miembros1){
  
    var listaOrdenada = miembros1.sort((primerMiembro, segundMiembro) => segundMiembro.missed_votes_pct - primerMiembro.missed_votes_pct);
    var filtrados = [];
    var limite = 10;
    var i = 0;
    while (i < limite || listaOrdenada[i].missed_votes_pct == listaOrdenada[i-1].missed_votes_pct){
        filtrados.push(listaOrdenada[i]);
        i++;
    }
...
function cargarTabla(){
  var dataJson = bottomDiez(miembros);
  ...
  for (i=0; i < dataJson.length; i++){
      if (dataJson[i].middle_name != null){
          fullName = dataJson[i].first_name + " "
          + dataJson[i].middle_name + " "
      ...
      $ ("#bottom").append(
            '<tr>'
          + '<td>' + fullName.link(dataJson[i].url) + '</td>'
          + '<td>' + dataJson[i].missed_votes + '</td>'
          + '<td>' + dataJson[i].missed_votes_pct+ '</td>'
          + '</tr>');
  }
}
cargarTabla ();
// ordenar por campos y mostrar con map
//members1.sort((primerMiembro, segundoMiembro) => primerMiembro.votes_with_party_pct - segundoMiembro.votes_with_party_pct).map(member => member.votes_with_party_pct)


//document.getElementById("id-sds").innerHTML(
//"<div>" + sumatoriaDeVotesWithPartyR/members1.length)

//while ( i< limite || members[i]. vipo == members[+1]) para mas adelante

//var result = listaFiltrada(function(a, b) {return a.votes_with_party_pct + b.votes_with_party_pct});
...
//console.log(Object.keys(statistic).length);

/*function suma1( datos) {
  var i;
  var total = 0;
...
const suma = Object.values(statistic).reduce((a, b) => a+b);


console.log(suma);

  var lista = members1.map(function(memb) {
    
    lista = memb.votes_with_party_pct
    
    return lista;
  });

var resultado = lista.reduce((a, b) => a+b);

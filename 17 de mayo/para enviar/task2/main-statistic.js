...
function principal(){
  ...
  statistic.number_of_independents = members.filter(miembro => miembro.party == 'I').length;
  statistic.number_of_democrats = members.filter(miembro => miembro.party == 'D').length;
  statistic.numer_of_republicans = members.filter(miembro => miembro.party == 'R').length;
  statistic.total = members.length;

  statistic.mostLoyal = getPercentList("votes_with_party_pct", "DES");
  statistic.leastLoyal = getPercentList("votes_with_party_pct", "ASC");
  statistic.mostEngaged = getPercentList("missed_votes", "DES");
  statistic.leastEngaged = getPercentList("missed_votes", "ASC");

  
statistic.average_votes_I = promedio("I");
statistic.average_votes_D = promedio("D");
statistic.average_votes_R = promedio("R");
statistic.average_votes_total = parseFloat(promedio("I")) + parseFloat(promedio("R")) + parseFloat(promedio("D"));
  
  function promedio(party) {
  return (members.filter(miembro => miembro.party == party).map(miembro => miembro.votes_with_party_pct).reduce((primerMiembro, segundoMiembro) => primerMiembro + segundoMiembro, 0) / members.length).toFixed(2);
  };

  
function getPercentList(key, order) {
  var listaOrdenada;
  if (order == "DES") {
    listaOrdenada = members.sort((primerMiembro, segundMiembro) => segundMiembro[key] - primerMiembro[key]);
  } else if (order == "ASC") {
    listaOrdenada = members.sort((primerMiembro, segundMiembro) => primerMiembro[key] - segundMiembro[key]);
  }
...
function porcentaje(array) {
  return parseInt(array.length / 10);
}

  var limite = porcentaje(members);

  var filtrados = [];

  var i = 0;
  while (i < limite || listaOrdenada[i][key] == listaOrdenada[i - 1][key]) {
...
function createTableAttendance(array) {
  ...
  for (var i = 0; i < array.length; i++) {
    ...
    attendanceSt += "<tr class=\"row\"><td class =\"col\">" + getName(array[i]) + "<\td><td class =\"col\">" + array[i].missed_votes +
      "<\td><td>" + array[i].missed_votes_pct + "<\td><\tr>";
  }
  return attendanceSt;
}
...
document.getElementById("bottom-at").innerHTML= createTableAttendance(statistic.leastEngaged);
  }

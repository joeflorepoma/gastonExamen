...
var members = dataHouse.members;

console.log(members.sort((primer, segundo) => primer.missed_votes - segundo.missed_votes));

function getHeaderHtml() {
	return document.getElementById("table-headers").innerHTML = "<tr><th>Full_name</th><th>party</th><th>state</th><th>seniority</th><th>percentage of votes with party</th></tr>";
};
getHeaderHtml();

function getColumnsHtml(data) {
	
	var tabla = data.map(function(memb) {
      
    var middleName = memb.middle_name || "";
      
	return '<tr><td><a href="' + memb.url + '">' + memb.first_name + ' ' + middleName + ' ' + memb.last_name + '</td><td>' + memb.party + '</td><td>' + memb.state + '</td><td>' + memb.seniority + '</td><td>%' + memb.votes_with_party_pct + '</td></tr>';
		}).join("");
...
function dropDown(members){
  var members = members.map(member => member.state);
  console.log(members);
...
function filtro(tabla){
  ...
  var checked = Array.from(document.querySelectorAll('input[name=party-filter]:checked')).map(elt => elt.value);
	var state = document.querySelector('select').value;
  console.log(state);
	var filtro = tabla.filter(tab => checked.includes(tab.party)&&(state == ''? true: tab.state == state));
